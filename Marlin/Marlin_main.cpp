...
void loop()
{
  ...
  if(buflen)
  {
	...
	} else { //сброс до следующего задания
		FC_Flag = false;
	}
  ...
  while( !card.eof()  && buflen < BUFSIZE && !stop_buffering) {
       ...
       serial_count >= (MAX_CMD_SIZE - 1)||n==-1)
    {
      if(card.eof()){
        ...
        SERIAL_ECHOLN(time);
        //MG
		sprintf_P(time, PSTR("Печать %iч. %iмин."),hours, minutes);
		lcd_setstatus(time);
...
void process_commands()
{
  ...
  else if(code_seen('M'))
  {
	...
	case 4:
	    if (!laser_enable) {
			SERIAL_ECHO_START;
			SERIAL_PROTOCOLPGM("Init Laser First!\n");
		} else {
		laser_on = true;
	    WRITE(LASER_PIN, 1);
        }
...
case 404:  //M404 Enter the nominal filament width (3mm, 1.75mm ) N<3.0> or display nominal filament width 
	...
	plan_buffer_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[E_AXIS], homing_feedrate[Y_AXIS]/4, active_extruder);
       ...
       // delay(100);
		...
		//MG ретракт перед возвратом к месту продолжения печати
		#ifdef FILAMENTCHANGE_FINISHRETRACT
           target[E_AXIS]+=(-1)*FILAMENTCHANGE_FINISHRETRACT ;
		...
		#ifdef FILAMENTCHANGE_FINISHRETRACT
           target[E_AXIS]+=FILAMENTCHANGE_FINISHRETRACT + FILAMENTCHANGE_FINISHRETRACTADD ;
		   plan_buffer_line(lastpos[X_AXIS], lastpos[Y_AXIS], lastpos[Z_AXIS], target[E_AXIS], feedrate/60, active_extruder);
        #endif
	
	   //My - так правильно (иначе координата Е с ошибкой)
	...
	case 555: // M555 MG Magnum addons
    {
	  ...
	  // Бред - (code_seen('PFR')) - ищет первую? букву!
	  //if(code_seen('F'))
	  if(code_seen('F'))
      {
		switch((int)code_value())
      ...
      } else if(code_seen('D')) //* прямое подключение
	  {
		switch((int)code_value()) 
		{
        case 0:
	  ...
	  } else if(code_seen('LZ')) 
	  {
        ...
        case 0:
		  laser_enable = false;
		  init_laser();
		  SERIAL_PROTOCOLPGM("Laser DISABLED\n");
          break;
        case 1:
		  laser_enable = true;
		  init_laser();
		  SERIAL_PROTOCOLPGM("Laser ENABLED\n");
          break;
		}
	  } 
	  else if(code_seen('W'))
	  {
		if (!laser_enable) {
          ...
          case 0:
		    laser_on = false;
		    WRITE(LASER_PIN, 0);
		    SERIAL_PROTOCOLPGM("Laser OFF\n");
            break;
          case 1:
		    laser_on = true;
		    WRITE(LASER_PIN, 1);
		    SERIAL_PROTOCOLPGM("Laser ON\n");
            break;
		  }
		}
  ...
  else if(code_seen('T'))
  {
	...
	*/
    if(tmp_extruder != active_extruder) {
	 ...
	 #ifdef SW_EXTRUDER //MG Swith extruder
	 sw_do_change(tmp_extruder);
	 #else //MG no Swith extruder
      #ifdef DUAL_X_CARRIAGE
...
void sw_do_clean() {
	  /* static float retract = 5;
	   ...
	   static int x_poz = 246; // Позиция по второму экструдеру!
	   static int x_poz_2 = 235;
	   static int y_poz = 15;
	   static int y_poz_2 = 2;
	   static float e_target = current_position[E_AXIS];
	   
		// 1) move to position 2) flow 3)retract 4)return 5)unretract 

		  plan_buffer_line(x_poz, y_poz, current_position[Z_AXIS] + 1, current_position[E_AXIS], max_feedrate[X_AXIS], active_extruder);

		  plan_buffer_line(x_poz, y_poz, current_position[Z_AXIS], current_position[E_AXIS], max_feedrate[Z_AXIS], active_extruder); //tmp чтоб прилипало
		  
		  e_target = current_position[E_AXIS] + flow;
		  plan_buffer_line(x_poz, y_poz_2, current_position[Z_AXIS], e_target, flow_speed, active_extruder);
		  
		  e_target = current_position[E_AXIS] + flow - retract;
		  plan_buffer_line(x_poz, y_poz_2, current_position[Z_AXIS], e_target, retract_speed, active_extruder);
		  
		  plan_buffer_line(x_poz, y_poz_2, current_position[Z_AXIS]+1, e_target, max_feedrate[Z_AXIS], active_extruder); //tmp чтоб прилипало

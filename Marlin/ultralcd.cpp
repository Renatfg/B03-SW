...
static void sw_service_position() {
	...
	// Датчик включен = 0
	if (active_extruder == 0) {
		digitalWrite(SW_DIR_PIN, 1);
		WRITE(SW_EN_PIN, 1);
		while (READ(SW_T0_PIN) == 0) { 	}
	} else {
		digitalWrite(SW_DIR_PIN, 0);
		WRITE(SW_EN_PIN, 1);
		while (READ(SW_T1_PIN) == 0) { 	}
	}
	WRITE(SW_EN_PIN, 0);
}
#endif
...
// MG суперпауза
static void lcd_filament_change_late()
{
Filament_change_now = 1;
jam_detected = 2; //работает как монитор прутка, но сообщение другое
lcd_return_to_status();
}
...
static void lcd_control_mg_laser_settings_menu()
{
	START_MENU();
	//MENU_ITEM(back, MSG_CONTROL, lcd_control_menu);
	MENU_ITEM(back, MSG_MG_LASER_MENU, lcd_control_mg_addons_menu);
	//Включение функции
	MENU_ITEM_EDIT_CALLBACK(bool, MSG_MG_LASER_ENABLE, &laser_enable, init_laser);
	MENU_ITEM_EDIT_CALLBACK(bool, MSG_MG_LASER_ON_OFF, &laser_on, laser_on_off);
	END_MENU();
}
void init_laser() {
	...
	if (!laser_enable) {
		...
		float tmp2[]={160, 160, 5, 5};    // (mm/sec)
		long tmp3[]={1000,1000,100,300}; //DEFAULT_MAX_ACCELERATION
		for (short i=0;i<4;i++) 
		{
			//axis_steps_per_unit[i]=tmp1[i];  
			max_feedrate[i]=tmp2[i];  
...
void lcd_sdcard_menu()
{
    ...
    START_MENU();
    MENU_ITEM(back, MSG_MAIN, lcd_main_menu);
	MENU_ITEM(gcode, LCD_STR_REFRESH LCD_STR_REFRESH MSG_INIT_SDCARD LCD_STR_REFRESH LCD_STR_REFRESH, PSTR("M21"));
    card.getWorkDirName();
    if(card.filename[0]=='/')
    {
#if SDCARDDETECT == -1
...
void sw_do_calibrate() {
	...
	// в рабочую точку
	plan_buffer_line(100.0, 45.0, current_position[Z_AXIS] + 1, current_position[E_AXIS], manual_feedrate[X_AXIS], active_extruder);
	st_synchronize();
	current_position[X_AXIS] = 100;
	current_position[Y_AXIS] = 45;
	...
	while(!lcd_clicked()){
		if (encoderPosition != 0) {
		refresh_cmd_timeout();
		// +- encoderPosition -> направление вращения ручки, + - правильно
		current_position[X_AXIS] += float((int)encoderPosition) * 0.04;
		encoderPosition = 0;
	...
	while(!lcd_clicked()){
		if (encoderPosition != 0) {
		refresh_cmd_timeout();
		current_position[Y_AXIS] += float((int)encoderPosition) * 0.04;
		encoderPosition = 0;
		plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], 3000, active_extruder);
		delay(10);
		}
	...
	encoderPosition = 0;
	// зададим смещение экструдеров
	extern float extruder_offset[];
	extruder_offset[1] -= current_position[X_AXIS];
	extruder_offset[3] -= current_position[Y_AXIS];
...
void sw_do_calibrate_z() {
	...
	sw_calibrate_z_now = 2; // Высота 1 экструдера
	float temp_z_position;
	temp_z_position = current_position[Z_AXIS];
	while(!lcd_clicked()){
		if (encoderPosition != 0) {
		refresh_cmd_timeout();
		// +- encoderPosition -> направление вращения ручки, + - правильно
		temp_z_position -= float((int)encoderPosition) * 0.01;
		encoderPosition = 0;
		plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], temp_z_position, current_position[E_AXIS], 3000, active_extruder);
		delay(10);
		}
		manage_heater();
		manage_inactivity(true);
	...
	sw_calibrate_z_now = 3; // Высота 2 экструдера
	sw_do_change(1);
	lcd_return_to_status();
	...
	encoderPosition = 0;
	// сохраним смещение сопел
	extruder_offset[5] = current_position[Z_AXIS] - temp_z_position;
	acceleration=DEFAULT_ACCELERATION;
	max_xy_jerk=DEFAULT_XYJERK;
	Config_StoreSettings();

	// Поднимем перед хомингом
	plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS] + 2, current_position[E_AXIS], max_feedrate[Z_AXIS]/2, active_extruder);
	st_synchronize();
	
	sw_calibrate_z_now = 0;
	sw_do_change(0);
	lcd_setstatus(WELCOME_MSG);
	lcd_return_to_status();
}
